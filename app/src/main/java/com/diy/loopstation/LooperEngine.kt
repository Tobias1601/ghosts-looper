...
/** What the engine is doing recording-wise right now - used to drive UI indicators
 *  (the transport bar and the active track's button) with distinct colors. */
enum class RecordPhase { NONE, WAITING, RECORDING_FRESH, OVERDUBBING }
...
class Track(val index: Int) {
    ...
    /** Frame window (within rawCapture) that's currently active/committed - lets the
     *  Edit screen re-open showing where the in/out points actually are, instead of
     *  always resetting to the full range. */
    @Volatile var activeWindowStart: Int = 0
    @Volatile var activeWindowEnd: Int = 0
...
class LooperEngine {
    ...
    @Volatile var recordingPhase: RecordPhase = RecordPhase.NONE
    ...
    fun startRecording(track: Track, multiplier: Float = 1f, onFinished: (masterCandidateRaw: ShortArray?) -> Unit) {
        ...
        thread {
            ...
            if (wasFirstTrack) {
                ...
                var total = 0
                recordingPhase = RecordPhase.RECORDING_FRESH
                while (recording.get()) {
                ...
                track.activeWindowStart = 0
                track.activeWindowEnd = raw.size
                activeRecordingTrackIndex = -1
                recordingPhase = RecordPhase.NONE
                recording.set(false)
                onFinished(raw)
            } else {
                val wasEmpty = track.isEmpty
                val targetFrames = if (wasEmpty)
                ...
                recordingPhase = RecordPhase.WAITING
                val boundarySpan = if (wasEmpty) masterLoopLengthFrames else track.buffer.size
                waitForBoundary(rec, boundarySpan, readBuf)
                recordingPhase = if (wasEmpty) RecordPhase.RECORDING_FRESH else RecordPhase.OVERDUBBING

                var writePos = 0
                var totalWritten = 0
                // A *fresh* recording runs for exactly targetFrames then auto-stops -
                // no need to judge when to press stop yourself. An ongoing overdub on
                // an already-filled track keeps mixing across as many passes as you
                // like until you press stop, Boss-pedal style.
                while (recording.get() && (!wasEmpty || totalWritten < targetFrames)) {
                    val n = rec.read(readBuf, 0, readBuf.size)
                    if (n > 0) {
                ...
                track.activeWindowStart = 0
                track.activeWindowEnd = track.buffer.size
                activeRecordingTrackIndex = -1
                recordingPhase = RecordPhase.NONE
                recording.set(false)
                onFinished(null)
    ...
    fun finalizeMasterTrim(track: Track, startFrame: Int, endFrame: Int) {
        ...
        track.activeWindowStart = s
        track.activeWindowEnd = e
        track.isEmpty = false
    ...
    fun stopAll() { isPlaying = false; tracks.forEach { it.stopPlayback() } }
